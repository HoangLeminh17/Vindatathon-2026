...
Vindatathon-2026/
│
├── dataset.zip               # File nén chứa toàn bộ dữ liệu gốc (.csv) do BTC cung cấp
│
...
1. Giải nén file `dataset.zip` trực tiếp vào thư mục hiện tại. (Extract here)
2. Đảm bảo sau khi giải nén, thư mục `dataset/` xuất hiện cùng cấp với các file mã nguồn và chứa tất cả các file CSV của cuộc thi (`sales.csv`, `products.csv`, `orders.csv`, v.v. cùng `sample_submission.csv`).
